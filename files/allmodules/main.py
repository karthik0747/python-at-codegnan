...
from exceptions import QueueSystemError, InvalidInputError
import utils
...
def handle_register(system, file_manager):
    try:
...
def handle_serve_next(system, file_manager):
    try:
        service_type, sub_service = select_service_and_sub(system)
...
def main():
    ...
    while True:
        ...
        try:
            if choice == "1":
                handle_register(system, file_manager)
            elif choice == "2":
                handle_serve_next(system, file_manager)
            elif choice == "3":
